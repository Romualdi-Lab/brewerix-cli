...
def call_and_merge():
    ...
    call_and_merge_multiallelic_to_biallelic(output_vcf, output_haplo_caller, output_selector,
                                             biallelic_snp, multiallelic_snp, bams,
                                             genome, gatk="~/local/stow/gatk-4.0.4.0/gatk")


def call_and_merge_multiallelic_to_biallelic(output_vcf, output_haplo_caller, output_selector,
                                             biallelic_snp, multiallelic_snp, bams,
                                             genome, gatk="~/local/stow/gatk-4.0.4.0/gatk"):

    if not exists(output_haplo_caller):
        print("Running Haplotype Caller...")
        run_haplotype_caller(gatk, multiallelic_snp, bams, genome, output_haplo_caller)

    if not exists(output_selector):
        print("Running Selector...")
        run_select_variants(gatk, genome, output_haplo_caller,
    ...
    with TemporaryDirectory() as wdir:
        ...
        run_select_variants(gatk, genome, intermediate_file,
                        ["--select-type-to-exclude", "INDEL", "--restrict-alleles-to", "BIALLELIC"],
                        output_vcf)
